...
class geetest(object):
    ...
    def getUrl(self):
        challenge = self.geetest_register()
        BASE_URL = "api.geetest.com/get.php?gt="
        if len(challenge) == 32:
            url = "http://%s%s&challenge=%s" % (BASE_URL, self.CAPTCHA_ID, challenge)
            httpsurl = "https://%s%s&challenge=%s" % (BASE_URL, self.CAPTCHA_ID, challenge)
            return url
        else:
            return None
